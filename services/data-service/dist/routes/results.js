...
router.get('/', async (req, res, next) => {
    try {
        const { ins_lot, material, batch, plant, sample_no, queue } = req.query;
        const filters = [];
        const values = [];
        let idx = 1;
        if (ins_lot) {
        ...
        if (sample_no) {
            filters.push(`sample_no = $${idx}`);
            values.push(sample_no);
            idx++;
        }
        if (queue) {
...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        const { rows } = await db_1.pool.query(`SELECT * FROM moisture.result WHERE id = $1`, [id]);
        if (rows.length)
            res.json(rows[0]);
        else
...
router.post('/', async (req, res, next) => {
    try {
        ...
        const { rows } = await db_1.pool.query(`INSERT INTO moisture.result(ins_lot, material, batch, plant, sample_no, queue, result, statistics)
       VALUES($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`, [ins_lot, material, batch, plant, sample_no, queue, result, statistics]);
